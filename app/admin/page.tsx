...
// 임시 통계 데이터 (DB 연동 전)
const mockStats = {
  reservations: { total: 24, pending: 3 },
  contacts: { total: 15, unread: 5 },
  portfolios: 12,
  reviews: 8,
};
...
const menuItems = [
  {
    ...
    count: mockStats.reservations.pending,
  ...
  },
  {
    ...
    count: mockStats.contacts.unread,
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
          <div className="rounded-xl border border-border bg-muted p-6">
            ...
            <p className="text-3xl font-bold mt-1">{mockStats.reservations.total}</p>
            <p className="text-xs text-accent mt-2">
              {mockStats.reservations.pending}건 대기중
            </p>
          </div>
          <div className="rounded-xl border border-border bg-muted p-6">
            <p className="text-sm text-muted-foreground">총 문의</p>
            <p className="text-3xl font-bold mt-1">{mockStats.contacts.total}</p>
            <p className="text-xs text-accent mt-2">
              {mockStats.contacts.unread}건 미확인
            </p>
          </div>
          <div className="rounded-xl border border-border bg-muted p-6">
            <p className="text-sm text-muted-foreground">포트폴리오</p>
            <p className="text-3xl font-bold mt-1">{mockStats.portfolios}</p>
            <p className="text-xs text-muted-foreground mt-2">등록된 영상</p>
          </div>
          <div className="rounded-xl border border-border bg-muted p-6">
            <p className="text-sm text-muted-foreground">고객 후기</p>
            <p className="text-3xl font-bold mt-1">{mockStats.reviews}</p>
