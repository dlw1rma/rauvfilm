...
const ALL_CATEGORY = "___ALL___";

// DB의 한국어 카테고리 값을 번역된 라벨로 변환
function getCategoryLabel(category: string, translations: PortfolioTranslations): string {
  switch (category) {
    case "가성비형":
      return translations.budgetType;
    case "기본형":
      return translations.standardType;
    case "시네마틱형":
      return translations.cinematicType;
    default:
      return category;
  }
}

export default function PortfolioPageClient({ translations }: { translations: PortfolioTranslations }) {
  ...
  // 카테고리 목록 추출
  const categories = [
    { key: ALL_CATEGORY, label: translations.all },
    ...Array.from(new Set(portfolioItems.map(item => item.category))).map(cat => ({ key: cat, label: getCategoryLabel(cat, translations) })),
  ];

  // 필터링 (카테고리 + 검색)
  const filteredItems = portfolioItems.filter((item) => {
    // 카테고리 필터
    if (selectedCategory !== ALL_CATEGORY && item.category !== selectedCategory) {
      return false;
    }
    // 검색 필터
    if (searchQuery.trim()) {
      ...
      <div className="min-h-screen">
            ...
            <div className="absolute inset-0 z-10 flex items-center">
              <div className="container mx-auto px-6 md:px-12 lg:px-16">
                <div className="max-w-2xl">
                  ...
                  >
                    <p>{translations.heroDesc1}</p>
                    <p>{translations.heroDesc2}</p>
                    <p>{translations.heroDesc3}</p>
                    <p>{translations.heroDesc4}</p>
                  </motion.div>
                </div>
              </div>
            </div>
        ...
        <div className="py-20 px-4">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mb-8">
              ...
              {(searchQuery || selectedCategory !== ALL_CATEGORY) && (
                <p className="mt-3 text-center text-sm text-muted-foreground">
                  {translations.resultCount.replace('{count}', String(filteredItems.length))}
                </p>
              )}
            </div>
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">{translations.noPortfolios}</p>
            </div>
